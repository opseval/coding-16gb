...
# context-guard (mid-run context bounding) must land as entrypoint + core, or long runs go unguarded
if [ -e "$REPO/extensions/context-guard.ts" ]; then
  assert '[ -f "$HOME/.pi/agent/extensions/context-guard.ts" ]'          "context-guard entrypoint (.ts) copied"
  assert '[ -f "$HOME/.pi/agent/extensions/lib/context-guard-core.mjs" ]' "context-guard core (lib/*.mjs) copied"
fi
assert '[ -f "$HOME/.pi/agent/extensions/frontier-scaffold.ts" ]' "extension entrypoint (.ts) copied"
# the bash guardrail's rules live in a sibling core frontier-scaffold.ts imports — must ship or the
# extension fails to load (safety gate gone)
assert '[ -f "$HOME/.pi/agent/extensions/lib/bash-guard-core.mjs" ]'   "bash-guard core (lib/*.mjs) copied"
